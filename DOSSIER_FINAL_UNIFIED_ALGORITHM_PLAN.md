...
- practical memory tiering: working context, compressed evidence memory, and reusable case memory
- optional affect / negativity / peak-end filters as post-synthesis style controls rather than truth controls
...
- Fogg-style behavior loops and habit automation as the main control system
...
## Free generation, constrained structure

Keep the idea layer open:

- wide hypothesis beams
- cross-domain analogies
- mechanism invention
- counterfactual expansion

Keep the structure layer fixed:

- evidence packets as the only reasoning input
- subgraph-scoped synthesis instead of whole-corpus prompting
- explicit assumptions
- explicit disconfirming signals
- explicit score breakdown

This keeps invention behavior aggressive without collapsing into random novelty.
...
- working context: only the current task plan and active subgraph slice
- compressed evidence memory: evidence packets, atom summaries, contradiction notes, weak signals
- case memory: reusable motifs, prior mechanism patterns, and prior high-yield hypothesis templates

Rules:

- never send the raw full corpus into `ADEPT-DS`
- retrieve by subgraph relevance and novelty potential, not by page order
...
- tool failure or sparse retrieval after a reasonable search pass
- uncertainty spikes or disagreement among top candidate mechanisms
- strong analogical matches that imply a high-novelty opportunity

No trigger: do not spend deep reasoning tokens.

Operational rule:

- treat contradiction, surprise, failure, and analogical opportunity as the main escalation signals for expensive reasoning
...
## Optional invention-style filters

These are selection or presentation filters, not truth gates.

Use only when they improve idea generation:

- affect weighting to surface themes that feel important or consequential
- negativity or loss weighting to sharpen conflict, downside, or strategic pressure
- peak-end retention to decide which invented motifs get stored back into case memory

Rule:

- never let these filters override the evidence graph or the minimal coherence constraints
...
- `GPT Researcher` / `Open Deep Research` as benchmark baselines, not as the core runtime
...
7. Grounded novelty:
- percent of top insights that are both semantically novel and traceable to at least two evidence clusters.

8. Trigger efficiency:
- whether expensive synthesis is mostly activated on real surprise, contradiction, failure, or strong analogical opportunity.

9. Memory reuse quality:
- whether case-memory motifs improve novelty-per-token instead of causing repetitive ideas.
